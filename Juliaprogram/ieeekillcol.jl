###for collision data without 2pi
...
function oneieeecol!(ven::Float64,ret::AbstractVector{Float64},i::Int64;d::Float64=0.05,fre::Float64=2.0e9,wb::Float64=0.0)
    ...
    er = 1-(ww-1im*ven)*(wpe.*wpe)/(w*(ww*ww+ven*ven))
...
function coldiff(;fre::Float64=2.0e9,d::Float64=0.05)
    ...
    PyPlot.plot(venx0,ret)#venx,ωₚ)

    f = open("Datacol$(fre/1e9)G.csv","w")

    write(f,"ve(GHz)\twp(GHz)\tA1(dB)\n")

    for i = 1:velen
        write(f,"$(venx[i]/1e9)\t$(ωₚ[i]/1e9)\t$(ret[i])\n")
